var About, App, Contact, Event, Home, Intro, Loader, Nav, Router, Scroll, SocialSharing, Transitions, UI_Slider, Utils, VideoPlayer, VideoPlayer_Controls, VideoPlayer_HTML, VideoPlayer_Templates, W,
...
Nav = (function() {
  function Nav() {
    this._closePopins = __bind(this._closePopins, this);
    this._closePopin = __bind(this._closePopin, this);
    this._displayPopin = __bind(this._displayPopin, this);
    this._onLinkClick = __bind(this._onLinkClick, this);
    this._initEvents = __bind(this._initEvents, this);
    this.container = $('#main-header');
    this.links = this.container.find('span');
    this.popins = $('.popin');
    this.popinsOverlay = $('.popins-overlay');
    this.popinsTimer = [];
    this.popinsId = [];
    this.links.each((function(_this) {
      return function(key, link) {
        return _this.popinsId.push($(link).attr('data-link'));
      };
    })(this));
    this._isOnePopinDisplayed = false;
    this._initEvents();
  }

  Nav.prototype._initEvents = function() {
    this.links.on(Event.CLICK, this._onLinkClick);
    return this.popins.on(Event.CLICK, '.close-button', this._closePopins);
  };

  Nav.prototype._onLinkClick = function(e) {
    var $this, link;
    $this = $(e.currentTarget);
    link = $this.attr('data-link');
    if (this.currentPopinId !== link) {
      $this.addClass('on').siblings().removeClass('on');
      this.currentPopinId = link;
      return this._displayPopin();
    }
  };

  Nav.prototype._displayPopin = function(id) {
    var popin;
    popin = $('#' + this.currentPopinId);
    Utils.clearTimers(this.popinsTimer);
    this._closePopin();
    window.scrollTo(1, 0);
    popin.removeClass('on-top').css({
      'top': 0,
      'display': 'block'
    });
    popin[0].offsetHeight;
    popin.css('opacity', 1);
    if (this._isOnePopinDisplayed === false) {
      this._isOnePopinDisplayed = true;
      this.popinsOverlay.css('display', 'block');
      this.popinsOverlay[0].offsetHeight;
      return this.popinsOverlay.css('opacity', 1);
    }
  };

  Nav.prototype._closePopin = function() {
    var popin;
    popin = null;
    $.each(this.popinsId, (function(_this) {
      return function(key, id) {
        if (id !== _this.currentPopinId) {
          return popin = $('#' + id);
        }
      };
    })(this));
    popin.addClass('on-top');
    popin.css({
      'opacity': 0,
      'top': $(window).scrollTop()
    });
    return this.popinsTimer.push(setTimeout((function(_this) {
      return function() {
  ...
  Nav.prototype._closePopins = function() {
    this.links.removeClass('on');
    this.currentPopinId = null;
    window.scrollTo(1, 0);
    $.each(this.popinsId, (function(_this) {
      return function(key, id) {
        var popin;
        ...
        return _this.popinsTimer.push(setTimeout(function() {
          return popin.css('display', 'none');
        }, 700));
      };
    })(this));
    this._isOnePopinDisplayed = false;
    this.popinsOverlay.css('opacity', 0);
    return this.popinsTimer.push(setTimeout((function(_this) {
      return function() {
        return _this.popinsOverlay.css('display', 'none');
      };
    })(this), 700));
  };
  ...
  function About() {
    ...
    if (W.device === 'desktop') {
      this.player = new VideoPlayer({
        container: this.container.find('.about-video'),
        autoplay: true,
        loop: true
      });
    } else {
      this.container.find('.about-video').replaceWith('<img class="frame" src="' + this.container.find('.about-video').attr('data-poster') + '" />');
    }
  }
...
Contact = (function() {
  function Contact() {
    ...
    if (W.device === 'desktop') {
      this.player = new VideoPlayer({
        container: this.container.find('.contact-video'),
        autoplay: true,
        loop: true
      });
    } else {
      this.container.find('.contact-video').replaceWith('<img class="frame" src="' + this.container.find('.contact-video').attr('data-poster') + '" />');
    }
  }
...
Home = (function() {
  function Home() {
    this._onAutoScroll = __bind(this._onAutoScroll, this);
    this._goTop = __bind(this._goTop, this);
    this._onListScrollUp = __bind(this._onListScrollUp, this);
    this._onListScrollDown = __bind(this._onListScrollDown, this);
    this._playProjectVideo = __bind(this._playProjectVideo, this);
    this._loadTexture = __bind(this._loadTexture, this);
    this._initAllVideos = __bind(this._initAllVideos, this);
    this._initEvents = __bind(this._initEvents, this);
    this._initContent = __bind(this._initContent, this);
    this.container = $('#home');
    this.viewportsContainer = this.container.find('.viewports-container');
    this.titlesMask = this.container.find('.titles-mask');
    this.titlesFront = this.container.find('.titles.front');
    this.numbersScroll = this.container.find('.scroll-attached-overview');
    this.numbersMask = this.container.find('.numbers-mask');
    this.numbersFront = this.container.find('.numbers.front');
    this._initContent();
    this._initAllVideos();
    this._initEvents();
  }
  ...
  Home.prototype._initContent = function() {
    ...
    this.worksScroll = new Scroll({
      container: this.container.find('.works')
    });
    this.listScroll = new Scroll({
      container: this.container.find('.list')
    });
    this.projectsNumber = [];
    this.container.find('.number-container').each((function(_this) {
      return function(key, number) {
        var item;
        item = {};
        item.elm = $(number);
        item.parent = item.elm.parent();
        item.startTop = item.parent.position().top;
        item.parentHeight = item.parent.height();
        item.texture = item.elm.siblings('.texture');
        return _this.projectsNumber.push(item);
      };
    })(this));
    this.numbersScrollHeight = this.numbersScroll.outerHeight();
    this.intro = new Intro();
    this._isListActive = false;
    this.currentVideoIndex = null;
    return this._loadTexture();
  };

  Home.prototype._initEvents = function() {
    $(this.listScroll).on(Scroll.DOWN, this._onListScrollDown).on(Scroll.UP, this._onListScrollUp);
    $('.go-top').on(Event.CLICK, this._goTop);
    return $(this.intro).on(Intro.AUTOSCROLL, this._onAutoScroll);
  };

  Home.prototype._initAllVideos = function() {
    this.works = this.container.find('article');
    this.videos = [];
    if (W.device === 'desktop') {
      return this.works.each((function(_this) {
        return function(key, work) {
          var item;
          item = {};
          item.startPos = $(work).offset().top;
          item.height = $(work).height();
          item.player = new VideoPlayer({
            container: $(work).find('.project-video'),
            loop: true
          });
      ...
      return this.works.each((function(_this) {
        return function(key, work) {
          var $this, video;
          $this = $(work);
          video = $this.find('.project-video');
          return video.replaceWith('<img class="frame" src="' + video.attr('data-poster') + '" />');
        };
      })(this));
    }
  };

  Home.prototype._loadTexture = function() {
    var img;
    img = new Image();
  ...
  Home.prototype._playProjectVideo = function() {
    var prevVideoIndex;
    prevVideoIndex = this.currentVideoIndex;
    this.currentVideoIndex = null;
    $.each(this.videos, (function(_this) {
      return function(key, video) {
        var _ref;
        if ((video.startPos + video.height > (_ref = _this.worksScroll.scrollTop.calc) && _ref > video.startPos)) {
          return _this.currentVideoIndex = key;
        }
      };
    })(this));
    if (prevVideoIndex !== this.currentVideoIndex) {
      if (this.currentVideoIndex !== null) {
        return this.videos[this.currentVideoIndex].player.play();
      }
    }
  };

  Home.prototype._onListScrollDown = function() {
    this._isListActive = true;
    clearTimeout(this.viewportsTimer);
    this.viewportsContainer.css('display', 'block');
    this.viewportsContainer[0].offsetHeight;
    this.viewportsContainer.css('opacity', 1);
    this.worksScroll.ease = 0.1;
    return this.resize();
  };

  Home.prototype._onListScrollUp = function() {
    this._isListActive = false;
    this.viewportsContainer.css('opacity', 0);
    this.viewportsContainer[0].offsetHeight;
    return this.viewportsTimer = setTimeout((function(_this) {
      return function() {
        _this.worksScroll.resetEase();
        return _this.viewportsContainer.css('display', 'none');
      };
    })(this), 500);
  };

  Home.prototype._goTop = function() {
    return this.worksScroll.goTop();
  };

  Home.prototype._onAutoScroll = function() {
    return this.worksScroll.goTo(W.wh, true);
  };
  ...
  Home.prototype.resize = function() {
    ...
    this.viewportsContainer.css('display', 'block');
    this.viewportsContainer[0].offsetHeight;
    this.worksScroll.resize();
    this.listScroll.resize();
    this.numbersScrollHeight = this.numbersScroll.outerHeight();
    $.each(this.projectsNumber, (function(_this) {
      return function(key, item) {
        item.startPos = item.parent.position().top;
    ...
    if (this._isListActive === false) {
      this.viewportsContainer.css('display', 'none');
    }
    scale = (2 * W.wh) / this.listScroll.overview.height();
    return scale = 2;
  };

  Home.prototype.update = function() {
    if (this._isListActive === true) {
      this.worksScroll.goTo(this.listScroll.scrollTop.perc * this.worksScroll.scrollTop.limit);
    } else {
      this.listScroll.goTo((this.worksScroll.scrollTop.calc / this.worksScroll.scrollTop.limit) * this.listScroll.scrollTop.limit);
    }
    this.worksScroll.update();
    this.listScroll.update();
    if (this._isListActive === true) {
      this.titlesMaskPos = (this.listScroll.overviewHeight - 82) * this.listScroll.scrollTop.perc;
      Normalize.transform(this.titlesMask[0], 'translate3d(0, ' + this.titlesMaskPos + 'px, 0)');
      Normalize.transform(this.titlesFront[0], 'translate3d(0, ' + (-this.titlesMaskPos) + 'px, 0)');
      this.numbersScrollPos = -((this.numbersScrollHeight - W.wh) * this.listScroll.scrollTop.perc);
      Normalize.transform(this.numbersScroll[0], 'translate3d(0, ' + this.numbersScrollPos + 'px, 0)');
      this.numbersMaskPos = (this.numbersScrollHeight - 350) * this.listScroll.scrollTop.perc;
      Normalize.transform(this.numbersMask[0], 'translate3d(0, ' + this.numbersMaskPos + 'px, 0)');
      Normalize.transform(this.numbersFront[0], 'translate3d(0, ' + (-this.numbersMaskPos) + 'px, 0)');
    }
    $.each(this.projectsNumber, (function(_this) {
      return function(key, item) {
        var pos, _ref;
        if ((item.startPos + item.parentHeight + W.wh > (_ref = _this.worksScroll.scrollTop.calc) && _ref > item.startPos - W.wh)) {
          pos = _this.worksScroll.scrollTop.calc - item.startPos;
          Normalize.transform(item.elm[0], 'translate3d(0, ' + pos + 'px, 0)');
          return Normalize.transform(item.texture[0], 'translate3d(0, ' + (pos * 0.8) + 'px, 0)');
        }
      };
    })(this));
    return this._playProjectVideo();
  };
...
Intro = (function() {
  ...
  function Intro() {
    this._onVideoCanPlay = __bind(this._onVideoCanPlay, this);
    this._onButtonClick = __bind(this._onButtonClick, this);
    this._initEvents = __bind(this._initEvents, this);
    this.container = $('#intro');
    if (W.device === 'desktop') {
      this.player = new VideoPlayer({
        container: this.container.find('.intro-video'),
        autoplay: true,
        loop: true
      });
    } else {
      this.container.find('.intro-video').replaceWith('<img class="frame" src="' + this.container.find('.intro-video').attr('data-poster') + '" />');
    }
    this._initEvents();
  }

  Intro.prototype._initEvents = function() {
    this.container.find('.button').on(Event.CLICK, this._onButtonClick);
    if (this.player) {
      return $(this.player).on(VideoPlayer.CANPLAY, this._onVideoCanPlay);
    }
  };

  Intro.prototype._onButtonClick = function() {
    return $(this).trigger(Intro.AUTOSCROLL);
  };

  Intro.prototype._onVideoCanPlay = function() {
    this.player.container.css('display', 'block');
    this.player.container[0].offsetHeight;
    return this.player.container.css('opacity', 1);
  };
  ...
  Intro.prototype.resize = function() {
    ...
    if (this.player) {
      return this.player.container.css(dims);
    }
  };
