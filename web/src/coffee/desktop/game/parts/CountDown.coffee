class CountDown
	...
	_drawProgressCircle: () =>
		...
		if pos < @arcZero + Math.PI * (1 - W.grid.bottomSpace)
			@ctx.arc(W.ww * 0.5, W.wh * 0.5, W.grid.radius + 20, @arcZero, pos, false)
		else
			@ctx.arc(W.ww * 0.5, W.wh * 0.5, W.grid.radius + 20, @arcZero, @arcZero + Math.PI * (1 - W.grid.bottomSpace), false)
		@ctx.stroke()

		@ctx.beginPath()
		if pos > @arcZero + Math.PI * (1 + W.grid.bottomSpace)
			@ctx.arc(W.ww * 0.5, W.wh * 0.5, W.grid.radius + 20, @arcZero + Math.PI * (1 + W.grid.bottomSpace), pos, false)
		else if pos == Math.PI * 2
			@ctx.arc(W.ww * 0.5, W.wh * 0.5, W.grid.radius + 20, @arcZero + Math.PI * (1 + W.grid.bottomSpace), @arcZero + Math.PI * 2, false)
		@ctx.stroke()
